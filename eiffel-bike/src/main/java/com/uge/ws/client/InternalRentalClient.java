...
public class InternalRentalClient extends JFrame {
    ...
    /** Simple ISO-style currency code check: 3 uppercase letters. */
    private boolean isCurrencyCodeFormatValid(String code) {
    ...
    private void loadShopBikes() {
        ...
        // --- NEW: read currency from text field ---
            ...
            try {
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    for (Map<String, Object> b : bikes) {
                        ...
                        // Prefer converted price if present, fallback to EUR
                        Object price = b.containsKey("price")
                                ? b.get("price")
                                : b.get("basePriceEUR");
    ...
    private void checkoutBasket() {
        ...
        // Read + normalise currency first
        String currencyInput = currencyField.getText().trim();
        if (currencyInput.isEmpty()) {
            currencyInput = "EUR";
        }
        // This one is effectively final and safe to capture in lambdas/threads
        final String targetCurrency = currencyInput;

        String url = baseUrlField.getText().trim()
                + "/shop/baskets/" + currentBasketId + "/checkout?currency=" + targetCurrency;
        appendLog("[POST] " + url);
